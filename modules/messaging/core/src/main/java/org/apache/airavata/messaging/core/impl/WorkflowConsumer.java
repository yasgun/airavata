...
public class WorkflowConsumer extends QueueingConsumer {
    ...
    public WorkflowConsumer(MessageHandler messageHandler, Connection connection, Channel channel) {
        super(channel);
    ...
    public void handleDelivery(String consumerTag, Envelope envelope, AMQP.BasicProperties properties, byte[] body)
        ...
        try {
            ...
            if (message.getMessageType() == MessageType.WORKFLOW_LAUNCH ||
                    message.getMessageType() == MessageType.WORKFLOW_CANCEL ||
                    message.getMessageType() == MessageType.WORKFLOW_PAUSE ||
                    message.getMessageType() == MessageType.WORKFLOW_RESTART) {

                TBase event = null;
                String gatewayId = null;

                WorkflowSubmitEvent workflowSubmitEvent = new WorkflowSubmitEvent();
                ThriftUtils.createThriftFromBytes(message.getEvent(), workflowSubmitEvent);
                log.info(" Message Received with message id: " + message.getMessageId() + " and with message type: " +
                        message.getMessageType() + " for workflowId: " + workflowSubmitEvent.getWorkflowId());

                event = workflowSubmitEvent;
                gatewayId = workflowSubmitEvent.getGatewayId();
